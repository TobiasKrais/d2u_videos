...
/**
 * REST API for the d2u_videos addon, built on top of the "api" addon.
 *
 * Registers a discovery endpoint plus generic CRUD endpoints for every
 * resource. All field handling is driven by {@see Schema}.
 */
final class VideosApi extends RoutePackage
{
    public function loadRoutes(): void
    {
        RouteCollection::registerRoute(
            'd2u_videos/schema',
            new Route('d2u_videos/schema', ['_controller' => self::class . '::handleSchema'], [], [], '', [], ['GET']),
            'd2u_videos: capabilities and writable field schema',
            null,
        ...
        foreach (Schema::getAvailableResources() as $resource) {
            ...
            RouteCollection::registerRoute(
                'd2u_videos/' . $resource . '/get',
                new Route(
                    'd2u_videos/' . $resource . '/{id}',
                    ['_controller' => self::class . '::handleGet', 'resource' => $resource],
                    ['id' => '\d+'], [], '', [], ['GET'],
                ),
                'd2u_videos: get a single ' . $resource . ' entry',
                null,
                new BearerAuth(),
                ['d2u_videos'],
            );
            ...
            RouteCollection::registerRoute(
                'd2u_videos/' . $resource . '/create',
                new Route(
                    'd2u_videos/' . $resource,
                    ['_controller' => self::class . '::handleCreate', 'resource' => $resource],
                    [], [], '', [], ['POST'],
                ),
                'd2u_videos: create a ' . $resource . ' entry',
                null,
                new BearerAuth(),
                ['d2u_videos'],
            );
            ...
            RouteCollection::registerRoute(
                'd2u_videos/' . $resource . '/update',
                new Route(
                    'd2u_videos/' . $resource . '/{id}',
                    ['_controller' => self::class . '::handleUpdate', 'resource' => $resource],
                    ['id' => '\d+'], [], '', [], ['PUT', 'PATCH'],
                ),
                'd2u_videos: update a ' . $resource . ' entry',
                null,
                new BearerAuth(),
                ['d2u_videos'],
            );
            ...
            RouteCollection::registerRoute(
                'd2u_videos/' . $resource . '/delete',
                new Route(
                    'd2u_videos/' . $resource . '/{id}',
                    ['_controller' => self::class . '::handleDelete', 'resource' => $resource],
                    ['id' => '\d+'], [], '', [], ['DELETE'],
                ),
                'd2u_videos: delete a ' . $resource . ' entry',
                null,
                new BearerAuth(),
                ['d2u_videos'],
            );
    ...
    {
        if ('playlists' === $resource && 'video_ids' === $field) {
            // Playlists store their videos as an array keyed by video id; save() only uses the keys.
